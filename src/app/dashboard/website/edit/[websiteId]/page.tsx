...
import { ArrowLeft, ImagePlus, LoaderIcon, Trash2, Users } from "lucide-react";
...
export default function EditWebsitePage() {
  ...
  useEffect(() => {
    if (websiteInfo) {
      reset({
        name: websiteInfo.name || undefined,
        email: websiteInfo.email || undefined,
        phone: websiteInfo.phone || undefined,
        logo: undefined,
        url: websiteInfo.url || undefined,
        organization: websiteInfo.organization || undefined,
        subject: websiteInfo.subject || undefined,
      });
    }
  }, [websiteInfo, reset]);

  useEffect(() => {
    if (website && websiteInfo && website.editLogoPreview == "") {
      website.setEditLogoFile(undefined);
      website.setEditLogoPreview(website.toLogoPreviewUrl(websiteInfo.logo));
      website.setEditId(websiteInfo._id);
    }
  }, [website, websiteInfo]);
      ...
      <div className="mx-auto max-w-2xl space-y-6">
        ...
        <div className="flex items-center gap-3">
          <Button
            variant="outline"
            size="icon"
            onClick={website.closeEditWebsite}
            className="border-border bg-card text-foreground hover:bg-accent"
          >
            <ArrowLeft className="h-4 w-4" />
          </Button>
          <div>
            <h1 className="text-2xl font-bold text-foreground">Edit Website</h1>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-8 shadow-lg shadow-slate-900/5">
          <form
            onSubmit={handleSubmit(website.onEditWebsite)}
            className="space-y-3"
          >
            <div className="rounded-xl border bg-[var(--background)] p-4">
              ...
              <div className="flex items-center gap-4">
                <div className="relative h-20 w-20 overflow-hidden rounded-full border bg-slate-200">
                  {website.editLogoPreview ? (
                    <div>
                      <Image
                        src={website.editLogoPreview}
                        alt="Logo Preview"
                        width={0}
                        height={0}
                      ...
                      />
                      <div className="hidden h-20 w-20 justify-center font-bold items-center">
                        <LoaderIcon />
                      </div>
                    </div>
                  ) : (
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      type="button"
                      variant="ghost"
                      disabled={!website.editLogoPreview}
                      onClick={() => website.onEditLogoFileChange(undefined)}
                    >
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Select
                    ...
                    value={field.value ?? websiteInfo.organization}
                  >
